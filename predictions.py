...
class PredictionWindow:
    ...
    def update_prediction_board(self, board_index: int):
        """Update the prediction board to reflect the board state of the selected prediction index and update the drawn board."""
    ...
    def update_compiled_prediction(self, compiled_board_state: fow_chess.FowBoard):
        """Update the compiled prediction board based on the current piece vision toggles and update the drawn board."""
    ...
    def create_compiled_prediction(self):
        ...
        piece_freq = {}  # Format: {tile : piece_type[]}
        self.compiled_prediction_board.clear_board() # Remove all pieces before adding pieces based on frequency in the predictions
        ...
        for board in self.BSL.board_states:
            for row in range(8):
                for col in range(8):
                    ...
                    if piece is not None and piece.color == chess.BLACK: # Only consider opponent pieces for the predictions
                        piece_freq[tile].append(piece)

        self.create_compiled_prediction_board(piece_freq) # Create the compiled prediction board based on the frequency of pieces in the predictions and the current piece vision toggles

    def create_compiled_prediction_board(self, piece_freq: dict):
        ...
        for tile, pieces in piece_freq.items():
            if len(pieces) > 0:
                if len(pieces) == 1:
                    self.compiled_prediction_board.set_piece_at(tile, pieces[0])
                else:
                    # NOTE: I think this is causing some errors, though it is just a placeholder until we implement outlines to indicate frequency
                    most_common_piece = max(set(pieces), key=pieces.count) # Get the most common piece on this tile across the predictions
                    self.compiled_prediction_board.set_piece_at(tile, most_common_piece)